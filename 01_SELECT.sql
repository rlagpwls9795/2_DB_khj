...
/* SELECT (SQL 또는 DML) : 조회
 * - 데이터를 조회(SELECT)하면 조건에 맞는 행들이 조회됨
 *   이 때, 조회된 행들의 집합을 "RESULT SET" (조회 결과의 집합)이라고 한다.
 * - RESULT_SET은 0개 이상의 행을 포함할 수 있다.
 */

-- [작성법]
-- SELECT 컬럼명 FROM 테이블명;  --> 어떤 테이블의 특정 컬럼을 조회한다.
...
SELECT EMP_NAME+10
FROM EMPLOYEE;
-- ORA-01722: 수치가 부적합합니다 --> 산술 연산은 숫자(NUMBER 타입)만 가능하다.
...
-- 주의 사항 2) DISTINCT 구문은 SELECT 제일 앞에 작성되어야 함
...
-- EMPLOYEE 테이블에서 급여가 300만원 초과인 사원의 사번, 이름, 급여, 부서 코드를 조회
...
-- NOT BETWEEN A AND B : A 이상 B이하가 아닌 경우 -> A 미만, B 초과
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN '1900-01-01' AND '1999-12-31'
...
-- LIKE의 ESCAPE OPTION : 일반 문자로 처리할 '_'나 '%'앞에 아무 특수기호를 첨부해서 구분
SELECT EMP_NAME, EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___#_%' ESCAPE '#'; --> # 뒤에 작성된 _는 일반 문자로 탈출
...
/* NOT IN */
-- EMPLOYEE 테이블에서 부서코드가 D1, D6, D9이 아닌 사원의 사번, 이름, 부서코드 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
-- EMPLOYEE 테이블 급여 오름 차순으로 사번, 이름, 급여 조회
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY;
