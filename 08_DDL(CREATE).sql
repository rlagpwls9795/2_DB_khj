...
/* 자료형
    ...
    DATE : 날짜 타입
    BLOB : 대용량 이진 데이터 (4GB) -> 동영상 이미지 등 (지양)
    CLOB : 대용량 문자 데이터 (4GB)
*/
...
-- SQL 작성법 : 대문자 작성 권장, 연결된 단어 사이에 _ (언더바) 사용
-- 문자 인코딩 UTF-8 : 영어, 숫자 - 1BYTE, 한글, 특수문자 - 3BYTE 취급
-- 만든 테이블 확인
SELECT * FROM MEMBER;
...
-- * INSERT/UPDATE 시 컬럼값으로 DEFAULT를 작성하면 테이블 생성 시 해당 컬럼에 지정된 DEFAULT값으로 삽입
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES('MEM02','QWER123','김영희','901231-2345678', SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO MEMBER VALUES('MEM03','ZXCV0987','박철수','980706-1122333', DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME)
VALUES('MEM04','MNBV678','이지연'); --주민번호-NULL, 가입일-현재시간

-- ** JDBC에서 날짜를 입력받았을 때 삽입하는 방법 **
-- '2022-09-13 17:33:27'
INSERT INTO "MEMBER" VALUES('MEM05','PASS05','김길동', '910202-1112222',
...
INSERT INTO MEMBER2 VALUES('MEM02','PASS02','고길순', 01034566789);
...
-- ORA-01400: NULL을 ("KH_KHJ"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- * 테이블 레벨 : 테이블 생성 시 컬럼 범위가 끝난 후 마지막에 작성
...
-- ORA-00001: 무결성 제약 조건(KH_KHJ.USER_ID_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_KHJ.USER_ID_NAME_U)에 위배됩니다
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_KHJ.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_KHJ"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_KHJ.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("KH_KHJ"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_FK(
  ...
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE/*(GRADE_CODE)*/ -- 컬럼레벨
  																	-- 컬럼명 미작성시 USER_GRADE의 PK를 자동 참조
  -- 테이블 레벨
...
-- ORA-02291: 무결성 제약조건(KH_KHJ.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
-- ORA-02292: 무결성 제약조건(KH_KHJ.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- ORA-02290: 체크 제약조건(KH_KHJ.GENDER_CHECK)이 위배되었습니다


-- CHECK 제약 조건은 범위로도 설정 가능.

-- ** CHECK 제약 조건은 FK 제약조건의 하위 호환
-- > 둘 다 컬럼의 값을 지정된 형태로만 제한을 하지만 
--   CHECK는 작성 후 수정이 번거롭지만 FK는 부모 테이블의 행을 추가, 수정 삭제가 간편
...
CREATE TABLE USER_TEST(
	...
	PNO VARCHAR2(20) CONSTRAINT NN_PNO NOT NULL,CONSTRAINT UK_PNO UNIQUE(PNO),
...
--> 서브쿼리로 테이블 생성 시 테이블의 형태(컬럼명, 데이터 타입)와 NOT NULL 제약조건만 복사되고
