...
-- SELECT절에 없는 컬럼이라도 FROM, JOIN으로 인해 존재하는 컬럼이면 ORDER BY 절에 사용 가능

-- 가장 적은 급여를 받는 직원의
-- 사번, 이름, 직급, 부서코드, 급여, 입사일을 조회
SELECT EMP_NO, EMP_NAME, JOB_NAME, SALARY, HIRE_DATE
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
SELECT NVL(DEPT_TITLE,'부서없음') "부서명", SUM(SALARY) "급여 합"
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID)
GROUP BY DEPT_TITLE
HAVING "급여 합"=17700000;
...
SELECT EMP_NAME, JOB_CODE, SALARY, 
	(SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE SUB
	 WHERE SUB.JOB_CODE=MAIN.JOB_CODE) "직급별 급여 평균"
FROM EMPLOYEE MAIN;
...
--> SELECT 해석 순서 때문에 급여 상위 5명이 아닌 조회 순서 상위 5명이 조회됨

--> 인라인 뷰 이용
-- 1) 이름, 급여를 급여 내림차순으로 조회한 결과를 인라인 뷰로 사용
      --> FROM절에 작성되기 때문에 해석 1순위
-- 2) 메인쿼리 조회시 ROWNUM을 5 이하 까지만 조회
SELECT ROWNUM, EMP_NAME, SALARY
