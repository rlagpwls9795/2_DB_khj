...
-- (참고) JOIN은 서로 다른 테이블의 행을 하나씩 이어붙이기 때문에 시간이 오래 걸리는 단점이 있다.
...
-- 컬럼 수를 기준으로 JOIN
--> 왼 편에 작성된 테이블의 모든 행이 결과에 포함되어야 한다 (JOIN이 안되는 행도 결과 포함)
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
-- TIP 같은 테이블이 2개 있다고 생각하고 JOIN 진행
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
NATURAL JOIN DEPARTMENT; --> 동일 컬럼이 없는 경우 교차 조인이 되어버림
...
--> 해결 방법 : DEPARTMENT와 LOCATION 조인 순서를 	바꿔서 EMPLOYEE와 DEPARTMENT가 조인된 결과를 먼저 만들어
