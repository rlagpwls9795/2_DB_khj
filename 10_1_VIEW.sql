...
-- OR REPLACE 확인 + 별칭 --
CREATE OR REPLACE VIEW V_EMP /*(사번, 이름, 부서명, 직급명)*/ AS
...
--> VIEW를 이용한 INSERT 시 VIEW가 아닌 원본 테이블에 삽입 진행

-- VIEW에 포함되지 않은 컬럼(DEPT_TITLE)에는 INSERT 시 NULL이 삽입
-- 만약 VIEW에 포함되지 않은 컬럼(DEPT_TITLE)에 NOT NULL 제약 조건이 설정되어 있는 경우
-- 정상적인 INSERT가 진행되지 않는다
--> ORA-01400: NULL을 ("KH_KHJ"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

-- VIEW의 원래 목적인 보여지는 것(조회)이라는 용도에 맞게 사용하는 것이 좋다
