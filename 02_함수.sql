...
-- 그룹 함수 : N개의 값을 읽어 1개의 결과를 반환 (합계, 평균, 최대, 최소 등)

-- 함수는 SELECT문의 SELECT절, WHERE절, ORDER BY절, GROUP BY절, HAVING절에서 사용 가능
...
---------------------------------------
-- INSTR(컬럼멸 | 문자열, '찾을 문자열'[, 찾기 시작할 위치 [,순번] ])
...
-- 컬럼이나 문자열에서 지정한 위치부터 지정된 길이만큼 문자열을 잘라내서 반환 (잘라 낸 부분만 남아 있음)
--> 잘라낼 길이 생략 시 끝까지 잘라냄
...
SELECT ROUND(123.456, 1) FROM DUAL; -- 123.5 --> 소수점 두 번째 자리에서 반올림 (=소수점 한 자리 표현)
SELECT ROUND(123.456, 0) FROM DUAL; -- 123 --> 소수점 첫 번째 자리에서 반올림 (=0이 기본 값)
SELECT ROUND(123.456, -1) FROM DUAL; -- 120 --> 소수점 0 번째 자리에서 반올림 (=소수점 -1자리 표현 = 10의 자리부터 표현)
SELECT ROUND(123.456, -2) FROM DUAL; -- 100 --> 소수점 -1 번째 자리에서 반올림 (=소수점 -2자리 표현 = 100의 자리부터 표현)
...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE, '2022-08-04'),3) "수강기간(개월)" 
FROM DUAL; --0.918
...
-- EXTRACT : 년, 월, 일 정보를 추출하여 리턴 (넘버타입으로 반환)
...
SELECT SYSDATE FROM DUAL; --2022-09-01 11:20:54.000

-- 2022/09/01 11:20:54 목요일
SELECT TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS DAY') FROM DUAL; 
SELECT TO_CHAR(SYSDATE, 'MM/DD (DY)') FROM DUAL; -- 09/01 (목)

-- 2022년 09월 01일 (목)
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" (DY)') FROM DUAL; 
-- ORA-01821: 날짜 형식이 부적합합니다 (년, 월, 일이 날짜를 나타내는 패턴으로 인식이 되지 않아서 오류 발생)
...
SELECT TO_DATE('2022-09-02') FROM DUAL; -- 문자열 --> DATE
SELECT TO_DATE(20220902) FROM DUAL; -- 숫자 --> DATE
...
-- EMPLOYEE 테이블에서 각 사원이 태어난 생년월일(1990년 5월 13일) 조회
SELECT EMP_NAME, TO_CHAR(TO_DATE(SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-')-1),'RRMMDD'),'YYYY"년" MM"월" DD"일"') 생년월일
FROM EMPLOYEE;
...
/*순서*/
/*3*/SELECT SUM(SALARY) "합계", ROUND(AVG(SALARY),0) "평균"
/*1*/FROM EMPLOYEE
...
SELECT COUNT(*) FROM EMPLOYEE; -- EMPLOYEE 테이블의 행 개수
SELECT COUNT(*) FROM EMPLOYEE WHERE BONUS IS NOT NULL; -- 보너스를 받는 사원 수
SELECT COUNT(BONUS) FROM EMPLOYEE;
SELECT DISTINCT DEPT_CODE FROM EMPLOYEE; --7행
SELECT COUNT(DISTINCT DEPT_CODE) FROM EMPLOYEE; -- 6 (NULL 제외한 실제 값이 있는 행의 개수만 조회)
