...
-- (그룹 별로 중간 집계 결과를 추가한다.)
-- GROUB BY절에서만 사용할 수 있는 함수
...
-- CUBE : GROUP BY절에 작성된 모든 컬럼의 집계를 처리하는 함수
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
...
- MINUS(차집합) : (집합 A-B) A에서 A,B 교집합 부분을 제거하고 조회
...
-- 주의
-- 집합 연산자를 사용하기 위한 SELECT문들은 조회하는 컬럼의 타입, 개수가 모두 동일해야 한다.
...
-- 서로 다른 테이블이지만 조회하는 컬럼이 같은 타입, 개수라서 집합 연산 가능 
